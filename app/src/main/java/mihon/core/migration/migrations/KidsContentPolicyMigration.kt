...
/**
 * Mihon Kids: clears every stored value that upstream used to decide which content ratings load.
 *
 * Replaces upstream's ContentWarningMigration, which migrated the legacy `show_nsfw_source` boolean
 * into `enabled_content_warnings`. This build reads none of these keys — mihon.kids.KidsPolicy
 ...
 *
 * Also covers an install that used to be upstream Mihon, or an earlier Mihon Kids build, and still
 * carries a permissive value in its preferences.
 */
@Inject
@ContributesIntoSet(AppScope::class)
class KidsContentPolicyMigration(
